...
**Status:** Executing
**Last activity:** 2026-02-24 — Completed 25-02-PLAN.md (types, config, and queries extraction to core)
...
v1.5 Core & CLI:    [██░░░░░░░░] ~10% - Phase 25 plan 2/5 complete
```

**Overall:** 85 plans complete across 24+ phases in 5 milestones.
...
**Date:** 2026-02-24
**Completed:** 25-02-PLAN.md — types, config, and queries extracted to packages/core/src/, all @/ imports fixed to relative ESM paths, tsc and tsdown builds verified
**Status:** Phase 25 plan 2/5 complete, ready for plan 3
...
Monorepo state after plan 25-02:
...
- `packages/core/tsdown.config.ts` — 4 entry points: index, types, config, queries
- `packages/core/package.json` exports — auto-updated by tsdown with all 4 subpath exports
- `bun run build` — builds core (dist/ with 4 subpath bundles) then web, both verified
- Web app still uses @/ imports from apps/web/src/ (rewiring deferred to later plans)
