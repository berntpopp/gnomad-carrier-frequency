...
Purpose: Enable users to install the app from Settings, see network status via a subtle offline indicator, and receive clear messaging when offline without cached data.

Output: usePwaInstall composable, useNetworkStatus composable, OfflineIndicator component, OfflineFallback component, Settings install section.
</objective>
...
<task type="auto">
  <name>Task 2: Create OfflineIndicator and OfflineFallback components, integrate into AppBar and StepGene</name>
  <files>src/components/OfflineIndicator.vue, src/components/OfflineFallback.vue, src/components/AppBar.vue, src/components/wizard/StepGene.vue</files>
  <action>
...
2. Create src/components/OfflineFallback.vue:
   - Import useNetworkStatus composable
   - Template: v-alert with v-if="!isOnline"
     - type="warning"
     - variant="tonal"
     - icon="mdi-wifi-off"
     - prominent (optional, for visual weight)
     - Text: "You're offline. Connect to use the calculator."
   - Add aria-live="polite" for screen reader announcement
   - Style: ensure it's visually distinct but not alarming

3. Update src/components/AppBar.vue:
...
4. Update src/components/wizard/StepGene.vue:
   - Import OfflineFallback component
   - Import useNetworkStatus composable
   - Add OfflineFallback at the top of the template (before gene search)
   - When offline (!isOnline), disable the GeneSearch component or show fallback prominently
   - The fallback message satisfies CONTEXT.md: "Simple fallback message when offline without cached data: 'You're offline. Connect to use the calculator.'"

The offline indicator is subtle (chip in AppBar). The fallback message is prominent (alert in wizard step) to clearly communicate why search won't work.
  </action>
  <verify>
- `bun run dev` shows app bar correctly
- Disconnecting network (DevTools offline mode) shows offline chip in AppBar
- StepGene shows "You're offline. Connect to use the calculator." alert when offline
- Reconnecting shows "Back online" snackbar briefly
  </verify>
  <done>OfflineIndicator displays in AppBar, OfflineFallback displays in StepGene when offline, back online notification shows</done>
</task>
...
3. Offline indicator visible in AppBar when network disconnected
4. StepGene shows "You're offline. Connect to use the calculator." when offline
5. "Back online" notification appears when reconnected
6. Settings > General shows Install App section
7. Install button triggers browser install prompt (if supported)
</verification>
...
- OfflineIndicator component shows subtle badge in AppBar when offline
- OfflineFallback component shows prominent message in StepGene when offline
