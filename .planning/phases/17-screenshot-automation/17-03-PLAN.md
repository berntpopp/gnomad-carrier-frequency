...
<task type="auto">
  ...
  <action>
    ...
    NOTE: This task captures 8 screenshots sequentially because the wizard flow is cumulative — each step builds on the previous step's state. Splitting would require duplicating navigation setup. The sequential nature is inherent to the wizard design.
    ...
    **2. step-1-gene-search (SHOT-03):**
    - We need the autocomplete dropdown visible again. The previous capture had the gene already selected.
    - Reset the autocomplete: First, try to find and click the clearable X button on the Vuetify v-autocomplete. Vuetify renders this as a `.v-field__clearable` button or an icon with `aria-label="Clear"`. Use: `await page.locator('[data-testid="gene-search-input"]').locator('.v-field__clearable').click()`. If the autocomplete uses a chip for the selected value, remove it first: `await page.locator('[data-testid="gene-search-input"]').locator('.v-chip__close').click()`.
    - After clearing, type "CFT" (partial) in the input to trigger the autocomplete dropdown. Use `await page.locator('[data-testid="gene-search-input"]').locator('input').fill('CFT')` (target the inner `<input>` element inside the Vuetify component, since the data-testid is on the wrapper).
    - Wait for the dropdown menu to appear with CFTR result. Look for `.v-autocomplete__content .v-list-item` or use `await page.locator('.v-overlay--active .v-list-item').waitFor()`.
    - Capture as `step-1-gene-search` while dropdown is visible
    - IMPORTANT: Do not click elsewhere between fill() and capture, as that would close the dropdown.
