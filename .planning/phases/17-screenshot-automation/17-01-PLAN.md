...
<task type="auto">
  ...
  <action>
    ...
    5. Create `fixtures/pinia/default-state.json` with Pinia persisted state entries:
       ...
       ```
       Note: `disclaimerAcknowledged` is set to `true` so the clinical disclaimer dialog does not appear during screenshots. This is the cleanest approach for ensuring all screenshots are captured without the modal overlay.
...
<task type="auto">
  ...
  <action>
    ...
    **2. Configuration constants:**
    ...
    ```
    The base path is confirmed: vite.config.ts has `base: '/gnomad-carrier-frequency/'` (line 109), so the dev server serves the app at `http://localhost:5173/gnomad-carrier-frequency/`.
    ...
    **6. Pinia localStorage injection:**
    Create `injectPiniaState(context: BrowserContext)` or do it via storageState when creating context. Load `fixtures/pinia/default-state.json` and set each key as a localStorage entry for the app's origin. The fixture has `disclaimerAcknowledged: true` so the disclaimer dialog will not appear in any screenshots.
...
- SHOT-17 satisfied (disclaimer auto-dismissed via localStorage injection with disclaimerAcknowledged: true)
