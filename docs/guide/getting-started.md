...
## Step 2: Set the Patient Status

The patient status describes the index patient's genetic findings. This step determines which recurrence risk formula the calculator uses, so it is important to select the correct option for your case.

There are four options:

- **Heterozygous carrier** — the index patient carries one pathogenic variant on one allele. This is the most common scenario in carrier screening. The recurrence risk depends on the partner also being a carrier, so the risk is calculated as: carrier frequency of the partner × 1/4 (the probability that both pass on their pathogenic allele). Expressed differently, the recurrence risk = partner carrier frequency ÷ 4.
- **Homozygous affected** — the index patient has two copies of the same pathogenic variant. One allele is already confirmed, so the risk formula uses a divisor of 2: recurrence risk = partner carrier frequency ÷ 2.
- **Compound heterozygous (confirmed)** — two different pathogenic variants are confirmed on separate alleles, verified by parental testing or long-read sequencing. The recurrence risk is the same as for homozygous affected: partner carrier frequency ÷ 2.
- **Compound heterozygous (assumed)** — two different pathogenic variants are detected but the phase (which variant is on which allele) has not been confirmed. The same risk formula applies as for confirmed compound heterozygous, but the generated clinical text notes this assumption.
...
**gnomAD tab** (default) — the calculator queries gnomAD directly for all pathogenic variants in your selected gene. A variant table shows each qualifying variant with its allele count, allele number, and allele frequency. Variants are pre-filtered for high-confidence loss-of-function (LoF HC) and ClinVar pathogenic/likely pathogenic entries. You can review the variant list and exclude specific variants if needed — for example, if a variant is known to be benign in your patient's population. See [Filters](/reference/filters) for a full description of how variants are selected.

**Literature tab** — enter a carrier frequency from a published study. This is useful when you have a known population-specific value from the literature, such as 1:25 for CFTR in Northern Europeans. You can optionally enter a PubMed ID, which will be cited in the generated clinical text.
...
Use the **copy button** to copy the text to your clipboard for pasting directly into a clinical letter or electronic health record system. The **share button** generates a URL that encodes your full calculation, which you can send to a colleague or save for reference.
...
The calculator is a Progressive Web App (PWA). After your first visit, the application is cached in your browser and continues to work without an internet connection — useful in clinical settings with unreliable connectivity. Note that offline mode uses the variant data cached from your last online session, so gnomAD queries require an internet connection when you need fresh data.
