...
The calculator generates clinical documentation text from customizable templates. This page is a complete reference for the template system — variable syntax, available perspectives, section structure, German gender-inclusive language, and customization options.

## Template System Overview

Templates are structured text documents with <span v-pre>`{{variable}}`</span> placeholders that are replaced with your calculation results. The calculator includes pre-built templates in German and English, each available in three clinical perspectives. You can customize templates through the settings dialog.
...
The template system includes three clinical perspectives, each generating text appropriate for a different patient or family situation.

### Affected Patient (`affected`)

For a patient with confirmed disease. The generated text:
- Describes the inheritance pattern and the gene-disease relationship
- Notes that both parents are likely carriers
- Presents the recurrence risk for future pregnancies based on the ÷2 formula

### Healthy Carrier (`carrier`)

For an identified carrier who is not affected. The generated text:
- Explains what carrier status means in plain clinical language
- Recommends partner testing
- Presents the combined reproductive risk if the partner is also a carrier (÷4 formula)

### Family Member (`familyMember`)

For a relative who may want cascade testing. The generated text:
...
| `recommendation` | Clinical recommendation | Testing recommendations for partner or family members |
...
- Maintain section separation — each section handles one aspect of the clinical narrative
...
- Keep the section's clinical purpose in mind — each section name describes its role (`geneIntro`, `recurrenceRisk`, etc.)

<figure class="screenshot-frame">
  <img src="/screenshots/text-output.webp" alt="Generated clinical text with all variables substituted and copy button visible" />
