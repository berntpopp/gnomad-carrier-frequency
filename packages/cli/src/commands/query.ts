...
export const queryCommand = new Command('query')
  ...
  .option('--clinical', 'Alias for --text (append clinical documentation text)', false)
  ...
  .option('--clinvar', 'Include ClinVar pathogenic/likely-pathogenic variants (default: true)')
  .option('--no-clinvar', 'Exclude ClinVar pathogenic/likely-pathogenic variants')
  .option(
    '--star-threshold <n>',
  ...
  .action(async (gene: string, opts: Record<string, unknown>) => {
    try {
      ...
      if (format === 'json') {
        output = formatJson(result, { includeVariants: opts['variants'] === true, pretty: true })
      } else if (format === 'tsv') {
        output = formatTsv(result, { includeVariants: opts['variants'] === true })
      } else {
        // Default: text
        output = formatText(result, { includeVariants: opts['variants'] === true })
      }

      // Append clinical text if --text or --clinical flag set
      if (opts['text'] === true || opts['clinical'] === true) {
        const language = typeof opts['language'] === 'string' ? opts['language'] : 'de'
      ...
      if (message.includes('not found in gnomAD')) {
        ...
        process.stderr.write(`Error: gnomAD server returned an error:\n  ${message}\n`)
      } else {
