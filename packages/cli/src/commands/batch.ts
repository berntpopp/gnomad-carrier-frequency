...
export const batchCommand = new Command("batch")
  ...
  .option(
    "-p, --population <id>",
    "Filter to a specific population (code or alias)",
  )
  ...
  .option(
    "--exclude-homozygotes",
    "Exclude homozygotes from carrier frequency (default)",
  )
  .option(
    "--no-exclude-homozygotes",
    "Include homozygotes in carrier frequency",
  )
  ...
  .action(async (file: string, opts: Record<string, unknown>) => {
    ...
    try {
      ...
      process.stderr.write(
        `[gnomad-cf] Error: Cannot read file "${file}": ${String(err)}\n`,
      );
      process.exit(2);
    }
    ...
    if (genes.length === 0) {
      process.stderr.write(
        "[gnomad-cf] Error: Gene list file is empty or contains no valid genes\n",
      );
      process.exit(2);
    }
    ...
    if (opts["gnomadVersion"] !== undefined)
      cliFlags["version"] = opts["gnomadVersion"];
    if (opts["hwe"] !== undefined) cliFlags["hwe"] = opts["hwe"];
    if (opts["excludeHomozygotes"] !== undefined)
      cliFlags["excludeHomozygotes"] = opts["excludeHomozygotes"];
    if (opts["penetrance"] !== undefined)
      cliFlags["penetrance"] = opts["penetrance"];
    ...
    const population = opts["population"]
      ? resolvePopulation(String(opts["population"]))
      : undefined;
    ...
    process.stderr.write(
      `[gnomad-cf] Processing ${genes.length} genes (concurrency: ${concurrency})\n`,
    );
    ...
    if (opts["output"]) {
      try {
        ...
        process.stderr.write(
          `[gnomad-cf] Output written to ${opts["output"]}\n`,
        );
      } catch (err) {
        process.stderr.write(
