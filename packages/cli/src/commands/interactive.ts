...
export async function runInteractive(): Promise<void> {
  ...
  if (searchResults.length === 0) {
    ...
    p.log.warn(
      `No gnomAD matches for "${geneRaw}" — will attempt query anyway`,
    );
  } else if (
    searchResults.length === 1 &&
    searchResults[0].symbol === geneRaw
  ) {
    ...
    options: [
      {
        value: "v4",
        label: "gnomAD v4.1",
        hint: "Recommended — largest dataset (~807K samples)",
      },
      {
        value: "v3",
        label: "gnomAD v3.1.2",
        hint: "Genome-only, includes Amish population",
      },
  ...
  const populations = await p.multiselect({
    message:
      "Select populations to display (space to toggle, enter to confirm):",
    ...
    options: [
      {
        ...
        hint: "Summary blocks per population",
      },
      {
        value: "json",
        label: "JSON",
        hint: "Machine-readable structured data",
      },
  ...
  try {
    result = await queryGene(gene, {
      ...
      population:
        selectedPopulations.length === 1 ? selectedPopulations[0] : undefined,
    });
