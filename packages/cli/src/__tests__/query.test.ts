...
describe("queryGene", () => {
  ...
  it('throws an error containing "not found" when gene data is null', async () => {
    ...
    await expect(queryGene("UNKNOWN", defaultOpts)).rejects.toThrow(
      /not found/i,
    );
  });
  ...
  it("throws an error when GraphQL errors are returned", async () => {
    ...
    await expect(queryGene("CFTR", defaultOpts)).rejects.toThrow(
      /gnomAD API error/,
    );
  });
  ...
  it("returns different variant count with LoF filter disabled", async () => {
    ...
    expect(resultDefault.variantCount).toBeGreaterThanOrEqual(
      resultNoLof.variantCount,
    );
  });
});
