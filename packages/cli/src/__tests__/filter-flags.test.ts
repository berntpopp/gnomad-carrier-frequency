...
describe('filter-flags integration', () => {
  beforeEach(() => {
    ...
    mockExecuteGraphQLQuery.mockResolvedValue(
      cftrFixture as ReturnType<typeof executeGraphQLQuery> extends Promise<infer T> ? T : never
    )
  })
  ...
  it('--no-lof --no-clinvar excludes all variants (count=0)', async () => {
    ...
    const result = await queryGene('CFTR', {
      ...baseOpts,
      filterConfig: { ...FACTORY_FILTER_DEFAULTS, lofHcEnabled: false, clinvarEnabled: false },
    })
  ...
  it('filter flags produce different globalCarrierFrequency values', async () => {
    ...
    expect(resultDefault.globalCarrierFrequency).not.toBe(resultNoClinvar.globalCarrierFrequency)
  })
