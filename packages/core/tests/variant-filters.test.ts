...
describe("shouldIncludeVariantConfigurable", () => {
  it("includes LoF HC when lofHcEnabled=true", () => {
    const variant = makeLofHCVariant("v1");
    expect(shouldIncludeVariantConfigurable(variant, [], defaultConfig)).toBe(
      true,
    );
  });
  ...
  it("includes missense with ClinVar evidence when missenseEnabled=true", () => {
    ...
    expect(shouldIncludeVariantConfigurable(variant, clinvar, config)).toBe(
      true,
    );
  });
  ...
  it("includes splice variant with ClinVar P/LP evidence", () => {
    ...
    const clinvar = [makeClinVarVariant("v1", "Pathogenic", 2)];
    expect(
      shouldIncludeVariantConfigurable(spliceVariant, clinvar, defaultConfig),
    ).toBe(true);
  });
  ...
  it("includes conflicting variant with 1 star even when clinvarStarThreshold is 2", () => {
    // Conflicting variants typically have 1 star. The gold_stars threshold
    // should NOT apply to conflicting variants — quality is ensured by the
    // submission-level P/LP percentage instead.
    const variant = makeGnomadVariant("v1");
    const clinvar = [
      makeClinVarVariant(
        "v1",
...
describe("filterPathogenicVariantsConfigurable", () => {
  ...
  // Regression: conflicting variants with 1 star must pass through
  // filterPathogenicVariantsConfigurable when clinvarStarThreshold=2.
  // Previously the gold_stars gate blocked them before submission check ran.
  it("includes conflicting variants in pipeline when clinvarStarThreshold > gold_stars", () => {
    const v1 = makeGnomadVariant("v1");
    const v2 = makeLofHCVariant("v2");
    const v3 = makeGnomadVariant("v3"); // no criteria — should be excluded
    ...
    expect(result.map((v) => v.variant_id)).toContain("v1");
    expect(result.map((v) => v.variant_id)).toContain("v2");
    expect(result.map((v) => v.variant_id)).not.toContain("v3");
  });

  it("excludes conflicting variants from pipeline when submissions below threshold", () => {
    const v1 = makeGnomadVariant("v1");
    ...
    // Only 1/5 P/LP = 20% — below 80% threshold
    const submissions: ClinVarSubmission[] = [
      { clinical_significance: "Pathogenic" },
      { clinical_significance: "Benign" },
      { clinical_significance: "Benign" },
  ...
  it("excludes conflicting variants when no submissions map provided", () => {
    const v1 = makeGnomadVariant("v1");
    ...
    // No submissions map — conflicting variant should NOT be included
    const result = filterPathogenicVariantsConfigurable([v1], clinvar, config);
    expect(result).toHaveLength(0);
  });
});
