...
async function navigateToStep4(page: Page): Promise<void> {
  ...
  await expect(page.getByRole('option', { name: /CFTR/ }).first()).toBeVisible({ timeout: 10_000 })
  ...
  await expect(page.getByText('Carrier frequency calculated from gnomAD data.')).toBeVisible({
    timeout: 15_000,
  })
...
test.describe('History restore', () => {
  test('completing a calculation and restoring from history shows Step 4 results', async ({
    ...
    await expect(page.getByTestId('step-results')).toBeVisible()
    await expect(page.getByTestId('results-summary-card')).toContainText('CFTR')
    ...
    await expect(page.getByTestId('step-gene')).toBeVisible({ timeout: 10_000 })
    ...
    await expect(page.getByTestId('history-drawer')).toBeVisible({ timeout: 5_000 })
    ...
    await expect(page.getByTestId('history-drawer')).not.toBeVisible({ timeout: 5_000 })
    ...
    await expect(page.getByTestId('step-results')).toBeVisible({ timeout: 15_000 })
