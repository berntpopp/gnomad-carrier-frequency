 ...
 */
function isVuetifyBtnDisabled(wrapper: ReturnType<typeof mountWithPlugins>, testId: string): boolean {
...
describe('StepFrequency', () => {
  ...
  it('renders the next button', () => {
    ...
    expect(wrapper.find('[data-testid="step-frequency-next-btn"]').exists()).toBe(true)
  })
  ...
  it('next button is enabled when gnomAD frequency is available', () => {
    ...
    expect(isVuetifyBtnDisabled(wrapper, 'step-frequency-next-btn')).toBe(false)
  })
  ...
  it('shows success alert text when gnomAD frequency is calculated', () => {
    ...
    expect(wrapper.text()).toContain('Carrier frequency calculated from gnomAD data.')
  })
