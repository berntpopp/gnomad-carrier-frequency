...
describe('StepStatus', () => {
  ...
  it('renders the next/continue button', () => {
    ...
    expect(wrapper.find('[data-testid="step-status-next-btn"]').exists()).toBe(true)
  })
  ...
  it('emits back event when back button is clicked', async () => {
    ...
    const backBtn = allBtns.find((b) => b.text().toLowerCase().includes('back'))
    ...
    if (backBtn) {
      ...
      const firstBtn = wrapper.findAll('[class*="btn"], v-btn').find((el) =>
  ...
  it('renders radio options with different status values', () => {
    ...
    const radios = wrapper.findAll('[value="heterozygous"], [value="homozygous"], [value="compound_het_confirmed"], [value="compound_het_assumed"]')
