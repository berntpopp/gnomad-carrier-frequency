...
export function useTextGenerator(input: () => TextGeneratorInput) {
  ...
  const templateContext = computed((): TemplateContext | null => {
    ...
    return {
      ...
      carrierFrequency: formatFrequencyForLocale(
        effectiveFrequency,
        store.language,
      ),
      carrierFrequencyRatio: formatRatio(effectiveFrequency),
      recurrenceRiskPercent: formatPercentForLocale(
        recurrenceRisk,
        store.language,
      ),
...
function formatSourceAttribution(
  ...
  switch (data.frequencySource) {
    ...
    case "default":
      return lang === "de" ? "(Standardannahme)" : "(default assumption)";
